...
COMMENT ON COLUMN tag.id         IS '标签ID，主键';
COMMENT ON COLUMN tag.user_id    IS '所属用户的 ID，外键，UUID 格式';
COMMENT ON COLUMN tag.name       IS '标签名称';
COMMENT ON COLUMN tag.color      IS '标签颜色';
COMMENT ON COLUMN tag.created_at IS '创建时间';
COMMENT ON COLUMN tag.updated_at IS '修改时间';

-- 资源标签关联表
CREATE TABLE resource_tag (
    id            BIGSERIAL PRIMARY KEY,
    tag_id        BIGINT NOT NULL REFERENCES tag(id) ON DELETE CASCADE,
    resource_type CHAR(1) NOT NULL CHECK (resource_type IN ('D','F')),
    resource_id   UUID NOT NULL,
    user_id       UUID NOT NULL,
    visibility    BOOLEAN NOT NULL DEFAULT TRUE,
    created_at    BIGINT NOT NULL DEFAULT TRUNC(EXTRACT(EPOCH FROM NOW()) * 1000)
);
COMMENT ON TABLE  resource_tag               IS '资源标签关联表';
COMMENT ON COLUMN resource_tag.id            IS '主键';
COMMENT ON COLUMN resource_tag.tag_id        IS '标签 id，外键引用 tag.id';
COMMENT ON COLUMN resource_tag.resource_type IS '资源类型，目录(D)或文件(F)';
COMMENT ON COLUMN resource_tag.resource_id   IS '资源 ID(文件 ID 或 目录 ID)， UUID 格式';
COMMENT ON COLUMN resource_tag.user_id       IS '冗余用户 ID，便于分区查询/权限校验';
COMMENT ON COLUMN resource_tag.visibility    IS '可见性，区分用户标签和系统内部标签';
COMMENT ON COLUMN resource_tag.created_at    IS '创建时间';

-- 资源标签关联表索引
CREATE INDEX idx_user_type_id ON resource_tag(user_id, resource_id);